...
pub const meta = types.InstructionMeta{
    .name = "BLENDVPS",
    .family = "BLEND",
    .source_path = "ISA/x86/BLEND/BLENDVPS.inc",
...
pub fn execute(comptime bits: usize, lhs: wide.Wide(bits), rhs: wide.Wide(bits), features: types.FeatureSet) types.SafetyError!wide.Wide(bits) {
    ...
    return types.SafetyError.UnsupportedInstructionWidth;
}

pub fn executeVariable(comptime bits: usize, lhs: wide.Wide(bits), rhs: wide.Wide(bits), selector: wide.Wide(bits), features: types.FeatureSet) types.SafetyError!wide.Wide(bits) {
    return instruction.blendVariable(bits, meta, lhs, rhs, selector, features);
}
