...
pub fn logFlags(scope: []const u8, sequence: u64, before_raw: u16, after_raw: u16) void {
    var event = bridge.makeFlagEvent(.dos, sequence, scope);
    ...
    runtime_abi.common.writeLine("[flag-trace][DOS] scope={s} seq={d} before=0x{x} after=0x{x}\n", .{ scope, sequence, before_raw, after_raw });
}
