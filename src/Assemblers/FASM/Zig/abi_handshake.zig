...
const fasm_core = @import("fasm_core.zig");
const model = @import("abi/model.zig");
const validate = @import("abi/validate.zig");
const runtime_abi = @import("runtime_abi_handshake");

pub const FasmAbiHandshake = struct {
    allocator: std.mem.Allocator,
    validator: validate.AssemblerValidator,
    sequence: u64 = 0,
    event_log: std.ArrayListUnmanaged(model.AssemblerEventRecord) = .{ .items = &.{}, .capacity = 0 },

    pub fn init(allocator: std.mem.Allocator) FasmAbiHandshake {
        runtime_abi.common.writeLine("[fasm][abi-handshake] init\n", .{});
        return FasmAbiHandshake{
            .allocator = allocator,
            .validator = validate.AssemblerValidator.init(allocator),
        };
    }

    pub fn deinit(self: *FasmAbiHandshake) void {
        for (self.event_log.items) |ev| self.allocator.free(ev.detail);
        self.event_log.deinit(self.allocator);
        runtime_abi.common.writeLine("[fasm][abi-handshake] deinit violations={d}\n", .{self.validator.error_count});
    }

    pub fn onEvent(self: *FasmAbiHandshake, event: model.AssemblerEvent, domain: model.ValidationDomain, address: u64, detail: []const u8) !void {
        self.sequence += 1;
    ...
    pub fn validateOutput(self: *FasmAbiHandshake, bytes: []const u8) void {
        self.validator.validateOutputSize(bytes.len, 1024 * 1024 * 16, "fasm output");
        if (bytes.len > 0) {
            self.validator.validateInstruction(bytes, 0);
        }
        if (self.validator.error_count > 0) {
            runtime_abi.common.violation("fasm-abivalidate", "output_validation_failed", "errors={d}", .{self.validator.error_count});
        }
    }

    pub fn flushEventLog(self: *FasmAbiHandshake) void {
        for (self.event_log.items) |ev| {
            runtime_abi.common.writeLine(
                "[fasm][abi][{s}] seq={d} pass={d} addr=0x{x} size={d} {s}\n",
...
test "abi handshake init deinit" {
    var h = FasmAbiHandshake.init(std.testing.allocator);
    defer h.deinit();
